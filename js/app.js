...
// 3回目。2回目で見えたことを、自分が決めた形にして試す回だけに置く。
// 任意なので、見出しと手順の両方で「ここで止めてよい」と伝える。
function deepenBlock(model) {
  const { before, after } = splitTemplate(model.deepen.template);
  const input = el("input", {
    type: "text",
    id: "deepen-input",
    className: "blank-input",
    value: deepenDraft,
    placeholder: model.deepen.hints[0] ?? "",
    autocomplete: "off",
    maxLength: 60,
  });
  const sentence = el("p", { className: "prompt prompt--fill", "data-prompt": "deepen" }, [
    el("span", { text: before }),
    input,
    el("span", { text: after }),
  ]);
  input.addEventListener("input", () => {
    deepenDraft = input.value;
  });

  return el("section", { className: "block block--fill block--deepen" }, [
    el("p", { className: "block__label" }, [
      createIcon(FILL_TYPE.icon),
      el("span", { text: `3回目（任意） — ${FILL_TYPE.label}` }),
    ]),
    stepList(model.steps.deepen),
    el("p", { className: "hints" }, [
      el("span", { className: "hints__label", text: "例：" }),
      ...model.deepen.hints.map((hint) => el("button", {
        type: "button",
        className: "hint",
        text: hint,
        "data-deepenhint": hint,
      })),
    ]),
    sentence,
    el("button", {
...
function renderDetail() {
  ...
  detailRoot.replaceChildren(
    ...
    el("div", { className: "stage-flow" }, [
      ...
      fillBlock(model),
      // 3回目を持つ回だけ、2回目の回答をどう読むかを挟んでから3回目を出す。
      model.hasDeepen ? el("p", { className: "reply" }, [
        createIcon("arrow"),
        el("span", {}, [el("span", { text: model.deepen.reply })]),
      ]) : null,
      model.hasDeepen ? deepenBlock(model) : null,
    ].filter(Boolean)),
...
function promptTextFor(name) {
  ...
  if (name === "deepen") {
    return challenge.deepen ? buildFilledText(challenge.deepen.template, deepenDraft) : "";
  }
...
document.addEventListener("click", (event) => {
  ...
  } else if (button.dataset.deepenhint) {
    const input = detailRoot.querySelector("#deepen-input");
    if (input) {
      input.value = button.dataset.deepenhint;
      deepenDraft = button.dataset.deepenhint;
      input.focus();
    }
